...
const getWhatsAppTemplates = async ({ limit = 10, offset = 0, search = '', category = '', status = '' }) => {
    let query = `
        SELECT t.*
    ...
    const parsedRows = rows.map(r => ({
        ...r,
        buttons: typeof r.buttons === 'string' ? JSON.parse(r.buttons) : r.buttons,
    ...
    const [categoryRows] = await pool.query(`SELECT DISTINCT category FROM whatsapp_templates WHERE status != 'deleted'`);
...
const getWhatsAppTemplateById = async (id) => {
    ...
    return {
        ...rows[0],
        buttons: typeof rows[0].buttons === 'string' ? JSON.parse(rows[0].buttons) : rows[0].buttons,
...
const createWhatsAppTemplate = async (data) => {
    ...
    const jsonButtons = buttons ? (typeof buttons === 'string' ? buttons : JSON.stringify(buttons)) : null;
    const jsonVars = variables ? (typeof variables === 'string' ? variables : JSON.stringify(variables)) : null;

    const [result] = await pool.query(`
        INSERT INTO whatsapp_templates 
        (tenant_id, template_name, template_key, category, dlt_template_id, header_type, header_content, message_body, footer_text, buttons, variables, status, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [tenant_id, template_name, generatedKey, category, dlt_template_id, header_type, header_content, message_body, footer_text, jsonButtons, jsonVars, status, created_by]);

    return getWhatsAppTemplateById(result.insertId);
};
...
const updateWhatsAppTemplate = async (id, data) => {
    ...
    const jsonButtons = buttons !== undefined ? (typeof buttons === 'string' ? buttons : JSON.stringify(buttons)) : undefined;
    ...
    await pool.query(`
        ...
        SET template_name = COALESCE(?, template_name),
            ...
            updated_at = NOW()
        WHERE id = ?
    `, [template_name, category, dlt_template_id, header_type, header_content, message_body, footer_text, jsonButtons, jsonVars, status, updated_by, id]);
