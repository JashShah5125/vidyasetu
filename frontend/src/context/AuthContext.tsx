  ...
  const login = async (email: string, password?: string): Promise<boolean> => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        const { token, refreshToken, sessionId, user } = response.data.data;
        ...
        if (sessionId) {
          localStorage.setItem('vs_session_id', sessionId);
        }
        ...
        const profile: UserProfile = {
          ...
          tenantName: user.tenantId === 1 ? 'Vidya Setu Platform' : 'Institute Name', // Could be fetched from backend
          branch: user.branch || '',
