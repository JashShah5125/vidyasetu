...
import {
  ...
  ChevronRight, ChevronLeft, ArrowLeft, Check, X, ChevronDown
} from 'lucide-react';
    ...
    <div className="flex flex-col gap-1.5 relative">
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center justify-between w-full h-11 px-3 border border-slate-200 rounded-lg bg-white text-sm cursor-pointer outline-none focus:ring-1 focus:ring-blue-500"
        >
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
            {options.map(opt => (
              <label key={opt} className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-50 cursor-pointer border-b border-slate-50 last:border-0 relative z-50">
                <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${selected.includes(opt) ? 'bg-blue-600 border-blue-600' : 'border-slate-300 bg-white'}`}>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-1 relative z-10">
          {selected.map(s => (
            <span key={s} className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded text-xs font-medium">
              {s}
              <button type="button" onClick={() => toggle(s)} className="hover:text-blue-900 opacity-70 hover:opacity-100 transition-opacity"><X size={12} /></button>
            </span>
          ))}
        </div>
      )}
              ...
              <div className="space-y-5">
                ...
                <FieldGrid>
                  <Select label="State *" value={form.state} onChange={e => {
                    ...
                    options={[{value:'',label:'Select State'},...Object.keys(INDIA_STATES_CITIES).map(s=>({value:s,label:s}))]} />
                  <Select label="City *" value={form.city} onChange={e => set('city', e.target.value)}
                    options={[{value:'',label:'Select City'},...(form.state && INDIA_STATES_CITIES[form.state] ? INDIA_STATES_CITIES[form.state] : []).map(c=>({value:c,label:c}))]} disabled={!form.state} />
              ...
              <div className="space-y-5">
                ...
                <FieldGrid>
                  ...
                  <Select label="Employment Type" value={form.employmentType} onChange={e => set('employmentType', e.target.value)}
                    options={[{value:'Full-Time',label:'Full-Time'},{value:'Part-Time',label:'Part-Time'},{value:'Visiting',label:'Visiting'}]} />
                </FieldGrid>
                <FieldGrid>
